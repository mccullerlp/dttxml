...
class DiagToDictNormalizer(declarative.OverridableObject):
    no_remap         = True
    channel_map      = {}
    channels         = None
    ...
    def read(
        ...
        channels = declarative.first_non_none(channels, self.channels)
        try:
        ...
        for idx_A in range(len(allchns_raw)):
            ...
            if chn_A_raw in self.channels_exclude:
                continue
            try:
                chn_A = self.channel_map[chn_A_raw]
            except KeyError:
                if self.no_remap:
                    chn_A = chn_A_raw
                else:
            ...
            if channels is not None and chn_A not in channels:
                continue
            if chn_A in self.channels_exclude:
                continue
            for idx_B in range(len(allchns_raw)):
                chn_B_raw = allchns_raw[idx_B]
                if chn_B_raw in self.channels_exclude:
                    continue
                try:
                    chn_B = self.channel_map[chn_B_raw]
                except KeyError:
                    if self.no_remap:
                        chn_B = chn_B_raw
                    else:
